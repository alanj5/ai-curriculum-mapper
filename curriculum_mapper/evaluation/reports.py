...
def generate_figures(results: dict) -> list[Path]:
    ...
    if per:
        ...
        x = range(len(modules))
        ax.bar([xi - 0.2 for xi in x], cov1, width=0.35, label="Coverage@1", color="#2196F3")
        ax.bar([xi + 0.2 for xi in x], cov3, width=0.35, label="Coverage@3", color="#4CAF50")
        ax.set_xticks(list(x))
        ax.set_xticklabels(modules, rotation=30, ha="right")
        ax.set_ylim(0, 1)
        ax.set_ylabel("End-to-End KA Coverage")
        ax.set_title("End-to-End KA Coverage per Annotated Module (extraction + alignment)")
        ax.legend()
